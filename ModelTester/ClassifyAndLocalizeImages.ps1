...
#############################
# Upcoming classifications
#############################

Start-Transcript -path "$($pathToTranscript)2017-08-22_vgg4_with_localization.txt" -append
python "$($pathToSourceRoot)ClassifyAndLocalizeImages.py" --classifier "2017-08-19_vgg4_with_localization.h5" --image_directory "$($pathToSourceRoot)test-data" 
Stop-Transcript
...
Start-Transcript -path "$($pathToTranscript)2017-08-19_vgg4_with_localization.txt" -append
python "$($pathToSourceRoot)ClassifyAndLocalizeImages.py" --classifier 2017-08-19_vgg4_with_localization.h5 --images "$($pathToSourceRoot)test-data\g-clef.jpg" "$($pathToSourceRoot)test-data\quarter-note.jpg" "$($pathToSourceRoot)test-data\Quarter-Note.png" "$($pathToSourceRoot)test-data\Half-Note.png" "$($pathToSourceRoot)test-data\G-Clef.png"
Stop-Transcript
